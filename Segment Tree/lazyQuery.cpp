...
int lazyQuery(int *tree,int ss ,int se,int qs,int qe,int index){
	...
	//Partial Overlap
	int mid=(ss+se)/2;
	int left=lazyQuery(tree,ss,mid,qs,qe,2*index);
	int right=lazyQuery(tree,mid+1,se,qs,qe,2*index+1);
...
void solve()
{
	...
	cout<<"Q 1-1  "<<lazyQuery(tree,0,n-1,1,1,1)<<endl;
	updateLazy(tree,0,n-1,3,4,10,1);
	cout<<"Q 3-5  "<<lazyQuery(tree,0,n-1,3,5,1);
